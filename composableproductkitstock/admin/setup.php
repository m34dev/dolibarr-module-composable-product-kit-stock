...
/**
 * \file    composableproductkitstock/admin/setup.php
 * \ingroup composableproductkitstock
 * \brief   ComposableProductKitStock setup page.
 */
...
// Translations
$langs->loadLangs(array("admin", "composableproductkitstock@composableproductkitstock"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
/** @var HookManager $hookmanager */
$hookmanager->initHooks(array('composableproductkitstocksetup', 'globalsetup'));
...
$moduledir = 'composableproductkitstock';
$myTmpObjects = array();
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/composableproductkitstock/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=composableproductkitstock-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'COMPOSABLEPRODUCTKITSTOCK_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'COMPOSABLEPRODUCTKITSTOCK_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
} elseif ($action == 'activateapimodule') {
	$resarray = activateModule('modApi');
	if (!empty($resarray['errors'])) {
		setEventMessages('', $resarray['errors'], 'errors');
	} else {
		setEventMessages($langs->trans("APIModuleEnabled"), null, 'mesgs');
	}
	header("Location: ".$_SERVER["PHP_SELF"].'?token='.newToken());
	exit;
}  elseif ($action == 'activateapilogin') {
	$res = dolibarr_set_const($db, 'API_DISABLE_LOGIN_API', '0', 'chaine', 0, '', $conf->entity);
	if ($res == 1) {
		setEventMessages($langs->trans("APILoginEnabled"), null, 'mesgs');
	} else {
		setEventMessages($langs->trans("EnableAPILoginError"), [], 'errors');
	}
	header("Location: ".$_SERVER["PHP_SELF"].'?token='.newToken());
	exit;
}
...
$help_url = '';
$title = "ComposableProductKitStockSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-composableproductkitstock page-admin');

// Subheader
$linkback = '<a href="'.($backtopage ? $backtopage : DOL_URL_ROOT.'/admin/modules.php?restore_lastsearch_values=1').'">'.$langs->trans("BackToModuleList").'</a>';

print load_fiche_titre($langs->trans($title), $linkback, 'object_composableproductkitstock@composableproductkitstock');

// Configuration header
$head = composableproductkitstockAdminPrepareHead();
print dol_get_fiche_head($head, 'Checks', $langs->trans($title), -1, "setup");
...
if ($action == 'edit') {
	...
	print $formSetup->generateOutput(false, true);
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'COMPOSABLEPRODUCTKITSTOCK_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'COMPOSABLEPRODUCTKITSTOCK_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
